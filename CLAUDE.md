...
- **IntersectionObserver NÃO dispara em aba `hidden`** (Chromium pausa o loop de render em background/janela coberta). Scroll-spy/reveal parece "quebrado" sob automação via claude-in-chrome com o Brave atrás de outras janelas — checar `document.visibilityState` ANTES de culpar o código; verificar via `agent-browser` (renderer próprio, sempre visível). Nav "Filiais" = scroll-spy (`lib/use-section-in-view.ts`), não hash — pushState do next/link não dispara `hashchange` (bug original da navbar, 07/2026).
