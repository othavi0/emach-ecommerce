...
4. CPF/CNPJ: validação no app (zod refine + dígito verificador em `packages/validators/src/cpf-cnpj.ts`, pacote `@emach/validators`). Sempre normalizar (só dígitos) antes de persistir em `client.document`.
...
- **IntersectionObserver NÃO dispara em aba `hidden`** (Chromium pausa o loop de render em background/janela coberta). Scroll-spy/reveal parece "quebrado" sob automação via claude-in-chrome com o Brave atrás de outras janelas — checar `document.visibilityState` ANTES de culpar o código; trazer a janela do Brave para a frente antes de medir. Nav "Filiais" = scroll-spy (`lib/use-section-in-view.ts`), não hash — pushState do next/link não dispara `hashchange` (bug original da navbar, 07/2026).
...
- **CI (`ci.yml`) tem 2 jobs.** `check-types` roda em série: `bun check` (lint ultracite) → `bun check-types` → `test:ci` do web (unit-only) → `test` de `@emach/validators`, `@emach/db` e `@emach/auth` (todos sem banco). `check-env` roda à parte. O mirror **não** espera o CI: `main` vermelha deploya igual. **`bun check:env`** (`scripts/check-vercel-env.ts`) cruza as env vars obrigatórias (derivadas do Zod em `packages/env/src/schemas.ts`) com a Vercel — **env obrigatória nova precisa ser cadastrada na Vercel** (`vercel env add`) senão o CI falha e o build quebraria no deploy.
...
Detalhe completo em `DESIGN.md`. **Vermelho é verbo, não decoração** — Ferrari Red (`#DA291C`) só em CTA de alta prioridade, UMA vez por tela. **Cantos quase retos = precisão** (`--radius: 2px` em interativos e avatares; checkbox 0; modais ≤8px; 50% só em indicador circular). Tipografia: **Barlow** (corpo) + **Barlow Condensed** (labels uppercase + tracking) — não misturar no mesmo bloco. Preços sempre `R$ 899,00`.
